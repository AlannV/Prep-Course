...
let str = "";
  ...
  for( let i = 0 ; i < stringSplit.length ; i++ ){
    ...
    if(stringSplit[i] == stringSplit[i].toUpperCase()){
      mayus.push(stringSplit[i]);
    }

    if(stringSplit[i] == stringSplit[i].toLowerCase()){
      minus.push(stringSplit[i]);
    }
  }

  str = mayus.join("").concat(minus.join(""));
  return str;
...
function deleteAbc(cadena){
  ...
  let replace = /a|b|c/gi;
  let str = cadena.replace(replace , "");
...
function sortArray(arr) {
  ...
  let arrayOrder = arr.sort(( a , b ) => a.length - b.length );
  return arrayOrder;
}
